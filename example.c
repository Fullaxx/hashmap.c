...
int main(int argc, char *argv[]) {
	...
	printf("\n-- get some users --\n");
	user = (struct user *)hashmap_get(map, &(struct user){ .name="Jane" });
	printf("%s age=%d\n", user->name, user->age);

	user = (struct user *)hashmap_get(map, &(struct user){ .name="Roger" });
	printf("%s age=%d\n", user->name, user->age);

	user = (struct user *)hashmap_get(map, &(struct user){ .name="Dale" });
	printf("%s age=%d\n", user->name, user->age);

	user = (struct user *)hashmap_get(map, &(struct user){ .name="Tom" });
